...
\label{sec:shuf}
The sign-balance result explains the main architecture-level separation, but it does not explain differences among controls that share the same global sign ratio. The degree-preserving shuffle experiments sharpen this interpretation. With the original real-valued \textit{C.\ elegans} weights, weight shuffling, connection shuffling, and connection-plus-weight shuffling all move the reservoir away from the unshuffled empirical connectome toward higher mean performance and higher CV (Figure~\ref{fig:realW}). This shift is not explained by magnitude placement alone. Connection shuffling produces CV increases comparable to weight shuffling for MC, IPC, and GR, and a larger increase for KR (Supplemental Table~\ref{tab:shuffle_cv_real}).

The ordering of the real-weight shuffle controls is informative. In a connection shuffle, weights move with their original outgoing edge source as edges are reassigned to new targets. In a weight shuffle, magnitudes are instead redistributed across the fixed empirical edge set, so source-specific patterns of outgoing synaptic strength are broken. The connection-plus-weight shuffle first disrupts the neighborhood structure and then breaks this source-associated magnitude pattern. Its lower performance and CV relative to the pure connection shuffle indicate that intrensic weight "movement'' that occurs whjen shuffling connections is not equivalent to an unconstrained shuffling of weights: keeping heterogeneous weights tied to their original source neuron can produce a higher-performance, higher-CV reservoir than randomizing those magnitudes independently. It also shows us that the effect of our topological shuffles is quite small in this setting, as the difference between the weight shuffle and weight+connection shuffle can be attributed purley to toplogical change. The real-weight shuffle results therefore point to an interaction between graph organization, sign placement, and heterogeneous magnitude placement. Disrupting either the empirical edge arrangement or the empirical assignment of magnitudes moves the reservoir out of the compact low-variance regime, but the way magnitudes are moved matters.

When all nonzero weights are collapsed to \(\pm 1\) while preserving the empirical sign ratio, the picture changes (Figure~\ref{fig:pm1W}). Using the original local sign placement as the baseline, the sign- and topology-shuffled signed-unit controls have lower CV across all four metrics (Supplemental Table~\ref{tab:shuffle_cv_pm1}). Preserving only the global sign ratio therefore recovers a lower-variance regime, but it does not make sign placement irrelevant; the original local sign arrangement places the signed-unit reservoir in a higher-performance, higher-CV regime than the shuffled signed-unit controls. The signed-unit weight-shuffle and connection-plus-weight-shuffle controls also occupy similar regions, the interpretation is the same as real-weighted results. The binary-weight experiment isolates topology further: once every nonzero edge has the same positive sign and magnitude, degree-preserving rewiring lowers CV most for MC (\(-10.2\%\)) and also for KR (\(-5.3\%\)), while IPC changes negligibly and GR changes more weakly (Figure~\ref{fig:bW}; Supplemental Table~\ref{tab:shuffle_cv_binary}).

These results give the hierarchy its second level. Global E/I sign imbalance is the strongest single control of the low-variance regime in the tested \textit{C.\ elegans} reservoir family, but the shuffle experiments show why smaller residual differences remain after sign ratio is fixed. Topology, local sign placement, and heterogeneous magnitude placement do not overturn the main sign-balance effect; instead, they determine where a reservoir falls within that sign-defined regime.
...
\section{Discussion}
We find a hyperparameter invariance--performance tradeoff dictated by the interaction between various network features. The \textit{C.\ elegans} connectome occupies a relatively low-variance reservoir regime, and the ablations point to an interwoven hierarchy of causes. Global E/I sign imbalance is the strongest single explanation in the model studied here: preserving the empirical sign ratio recovers much of the connectome's invariance even when exact sign placement and empirical weight magnitudes are altered. This hierarchy is conditional because sign balance does not act in isolation. Its apparent power depends on the topology it is placed on, the distribution of weight magnitudes, the placement of those magnitudes, and the placement of inhibitory connections.

Moving toward a more balanced E/I regime generally increases mean task-agnostic performance, but it also increases CV across spectral radius, leak rate, input scaling, and neuron bias. Under the present modeling choices, the empirical \textit{C.\ elegans} instantiation lies on the low-variance side of this curve rather than at the peak-performance region. Put more directly, the approximately 4:1 excitatory-to-inhibitory balance associated with many biological networks does not prioritize peak reservoir performance in this abstraction; it produces a more hyperparameter-invariant, lower-performance operating point. The result is therefore not a universal claim about E/I imbalance alone, but a statement about how this sign ratio acts in combination with the other structural choices in the present reservoir model.

\subsection{Shape of the sign-balance curves}

The approximate symmetry of the sign-fraction curves around a balanced edge-sign ratio is also consistent with the mathematical form of the reservoir used here. The state nonlinearity is \(\tanh\), which is odd-symmetric, and the readout is linear. Under these choices, exchanging positive recurrent weights for negative recurrent weights changes the sign structure of the recurrent drive but can be "corrected'' for using the linear readout, the "function'' of our network is therefore symetrical. A our results suggest the peak at 50:50 balance is a result of the suppression of recurrent feedback loops and by association, spectral radius.  A centered peak in performance and a corresponding increase in CV near balanced edge signs are therefore natural for this abstraction.
