...
\subsubsection{Omission of CANL/CANR}
\label{sec:omission}
...
\label{sec:ipc}
Information processing capacity measures how well the reservoir can reconstruct nonlinear functions of delayed inputs. For Legendre polynomial orders \(K=\{1,3,5\}\), we compute
\[
\mathrm{IPC}
=
\sum_{d=1}^{D_{\max}}
...
\subsection{Magnitude placement matters, but only after sign structure is fixed}
\label{sec:magnitude_interpolation}
\label{sec:weight_perturbation}
The preceding sections show that sign structure must be fixed before magnitude effects can be interpreted cleanly. The sign-preserving interpolation experiments therefore ask a narrower question: once topology and edge signs are held constant, does the placement of empirical magnitudes across signed edges matter?
...
Direct biological interpretation should be cautious because the connectome model compresses substantial uncertainty into a static signed network. In our instantiation, approximately $6\%$ of connections are inhibitory, but recent sign-inference work suggests the inhibitory fraction may be closer to $20\%$ \parencite{fenyves_szilágyi_zsolt_vassy_csaba_2020}. If so, the empirical connectome may lie further along the observed performance--CV curve than our current model suggests. In addition, E/I balance varies across development and aging \parencite{wirak_florman_alkema_connor_gabel_2022, tanis_bellemer_moresco_forbush_koelle_2009}, and our model uses a coarse neurotransmitter-to-sign mapping, treats non-GABA synapses as excitatory, and delivers inputs to all neurons rather than to biologically segregated populations. The present work should therefore be read as a structural reservoir analysis, not a faithful dynamical simulation of the animal.

For reservoir computing, the practical implication is that E/I sign ratio should be treated as a primary design parameter for controlling the tradeoff between robustness and task-agnostic performance. Topology and magnitude placement remain useful tuning dimensions, but their effects are more metric-specific: topology and heterogeneous weights matter most for MC, while magnitude placement especially affects KR and GR after sign structure is fixed.
...
This random-graph baseline uses \(N=299\) nodes and an adjusted ER edge set with the same number of directed edges as the \emph{C.~elegans} network. After fixing the edge count, weights are independently sampled from \(\mathcal{N}(0,1)\).

\subsubsection{Watts--Strogatz Gaussian control}
This small-world control uses \(N=299\) nodes and \(3108\) directed edges. A directed ring lattice is rewired with probability \(p=0.1\), preserving source out-degree and edge count, and nonzero weights are independently sampled from \(\mathcal{N}(0,1)\).
